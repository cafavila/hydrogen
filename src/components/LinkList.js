...
//export const FEED_QUERY = gql `{feed {links {id url description createdAt postedBy {id name} votes {user {id}}}}}`
export const FEED_QUERY = gql `query FeedQuery($first: Int, $skip: Int, $orderBy: LinkOrderByInput)
      {feed(first: $first, skip: $skip, orderBy: $orderBy) {links {id url description createdAt postedBy {id name} votes {id user {id}}} count}}`
...
class LinkList extends Component
{
  ...
  _subscribeToNewLinks = subscribeToMore => {
      console.log('NEWS - Subscripcion de noticias');
  ...
  render()
  {
      return (<Query query={FEED_QUERY} variables={this._getQueryVariables()}>
        {({loading, error, data, subscribeToMore}) => {
          if (loading) return <div>Cargando...</div>
          if (error) return <div>Error Inesperado :{error.message}</div>
